...
const generateInitPosition = () => {
    ...
    const distance = Math.random() * (CIRCLE_RADIUS_SMALL  - (DAMAGE_SIZE_INIT / 2));
...
const generateFinalPosition = (deltaX: number, deltaY: number) => {
    const distance = Math.max(CIRCLE_RADIUS_SMALL +  (Math.random() * ((CIRCLE_RADIUS_BIG - CIRCLE_RADIUS_SMALL) - DAMAGE_SIZE_FINAL / 2)), CIRCLE_RADIUS_SMALL + DAMAGE_SIZE_FINAL / 2)
